...
import { getPopularMovies, isTmdbConfigured } from "@/lib/tmdb/queries";

/** Grille Films (D19) — tri popularité ; facettes genres/année au fil du Lot 2+. */
...
export default async function FilmsPage({
  ...
  const data = isTmdbConfigured() ? await getPopularMovies(page) : null;
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
      ...
      {data && data.titles.length > 0 ? (
        <>
          ...
          <Pagination basePath="/films" page={data.page} totalPages={data.totalPages} />
        </>
      ) : (
        <EmptyState
          illustration="/media/interface/empty-search.jpg"
          title="Le catalogue arrive"
          description="Les films seront affichés dès que la source catalogue sera connectée. Revenez très vite !"
          actionHref="/"
